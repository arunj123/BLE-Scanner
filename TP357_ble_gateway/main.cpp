    ...
    // #include "FirestoreManager.h"      // REMOVED: Provides full definition of FirestoreManager
    ...
    int main(int argc, char **argv) {
        ...
        // EnvReader is still useful for other potential settings, but Firestore-specific ones are ignored.
        EnvReader env_reader;
        if (!env_reader.load(".env")) {
            std::cerr << "Could not load .env file. Using default settings." << std::endl;
        }

        // Get Firestore config path from .env or use a default placeholder // REMOVED: No longer needed
        ...
        // Create and initialize the Firestore manager (REMOVED)
        // auto firestore_manager = std::make_unique<FirestoreManager>(); // REMOVED
        // if (!firestore_manager->initialize(firestore_config_path)) { // REMOVED
        //     std::cerr << "Failed to initialize Firestore manager. Proceeding with SQLite only." << std::endl; // REMOVED
        // } // REMOVED

        // Example of simulating Firestore going offline/online (REMOVED)
        // firestore_manager->setSimulatedOnlineStatus(true); // REMOVED

        // Create the data processor, passing only the message queue and SQLite database manager
